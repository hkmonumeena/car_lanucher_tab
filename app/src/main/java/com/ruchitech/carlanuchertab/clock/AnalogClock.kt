...
fun AnalogClock() {
    Box(
        modifier = Modifier
            .size(380.dp), contentAlignment = Alignment.Center
    ) {
        var time by remember {
            mutableStateOf(
                Time(
                    hour = 0f, minute = 0f, second = 0f
                )
            )
        }
        ...
        Canvas(modifier = Modifier.size(380.dp)) {
            ...
            val textPaint = Paint().apply {
                ...
                isAntiAlias = true
                setShadowLayer(6f, 2f, 2f, android.graphics.Color.BLACK) // 👈 soft shadow for depth
            ...
            for (hour in 1..12) {
                val angleRad =
                    Math.toRadians((hour * 30 - 90).toDouble()) // Rotate -90 to start from top
                val x = center2.x + (radius * cos(angleRad)).toFloat()
                val y = center2.y + (radius * sin(angleRad)).toFloat()

                drawContext.canvas.nativeCanvas.drawText(
                    hour.toString(), x, y + 15f, // small vertical offset for better alignment
                    textPaint
                )
            }
            ...
            rotate(time.hour, pivot = center2) {
                ...
                drawPath(
                    path, color = Color.White, style = Fill
                )
                drawPath(
                    path, color = Color(0xFF3A5A78), style = Stroke(width = 1f)
                )
            }
